...
> 说明：当前 issue 无显式 assignee，先以可确认参与用户作为候选，待你确认后落定。

### 负责人层级 A（决策与验收）
- Owner：`NongHua123`（主协调）
- 任务：
  - 批准 `InstallPlan` 四层契约定义与字段字典；
  - 决定方案一中的字段命名规范与发布文案；
  - 在 GitHub 上发起 1 条主结论置顶。

### 负责人层级 B1（后端）
- Owner：`python-rust`（候选）
- 任务：
  - 扩展后端 DTO：`source_trust_state`、`package_trust_level`、`preflight_codes`；
  - 把 plan snapshot 与 `expected_release_id`/`RestartPlan` 对齐；
  - 给 `#28` 增加 drift invalidation 触发条件（source/version/hash/perm/path 变化）。

### 负责人层级 B2（前端）
- Owner：`farion1231`（候选）
- 任务：
  - UI 状态树改为四层显示（来源/完整性/环境/计划）；
  - `unknown` 直接可见、不可误渲染；
  - 点击展开显示技术细节（PATH、WSL、远端等）。

### 负责人层级 C（合规与验证）
- Owner：`待确认`
- 任务：
  - 对 6 款 agent 的来源与许可字段做可读化映射；
  - 建立手工重验清单（撤回、签名、权限）；
  - 与 #70 #74 #57 #29 的边界同步。
...
- 这份 PRD 已形成链路决策文档，请确认后我会：
  1) 同步发布到 GitHub（注释/任务备注）；
  2) 同步飞书（待定：若当前会话无飞书可写链路，先输出可直接粘贴正文）。
