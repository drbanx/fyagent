...
| `check:contracts:prearchive` | Run contract checks with one exact current Trellis task excluded before archival      | --exclude-active-task <path> | read-only              |
| `check:frontend`             | Run frontend type, formatting, unit, i18n, desktop mock, and visual preflight checks  | —                            | read-only              |
| `check:prearchive`           | Run the complete check with one exact current Trellis task excluded before archival   | --exclude-active-task <path> | read-only              |
