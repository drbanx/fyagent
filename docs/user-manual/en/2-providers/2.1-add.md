...
- Download and open the repository's bundled [`deplink.html` generator](https://github.com/NongHua123/fyagent/blob/main/deplink.html) locally
...
FyAgent provides a **Codex OAuth reverse proxy** path that lets you reuse your ChatGPT account's Codex service inside Claude Code.
...
Codex OAuth model selection does not rely only on a hardcoded list. When the model selector opens, FyAgent fetches available models from the ChatGPT Codex backend on demand; the default mapping can still be overridden.
...
FyAgent supports **Full URL Endpoint Mode**. By default, it treats the configured `base_url` as a **prefix** and appends fixed paths like `/v1/chat/completions`. For some vendors (such as third-party services with non-standard URL layouts), this path concatenation causes requests to fail.
...
The auto-compact limit can be customized in the text field that appears when the toggle is on. This toggle appears only when adding a new Codex provider; when editing an existing provider, adjust the fields directly in advanced configuration if needed.
