...
The complete app is ad-hoc signed with no certificate identity; it is not
signed with an Apple Developer ID and is not notarized, while the DMG container
is unsigned. Ad-hoc signing does not provide Apple trust. Download only from the
[Releases page](https://github.com/NongHua123/fyagent/releases), verify the
exact build's evidence, and follow the safe
[Gatekeeper Open Anyway procedure](../1-getting-started/1.2-installation.md#if-gatekeeper-blocks-the-first-launch).
Do not disable Gatekeeper or remove the file's quarantine attribute.
...
```bash
# x64: add execute permission
chmod +x FyAgent-X.Y.Z-Linux-x86_64.AppImage

# x64: if it still fails, try
./FyAgent-X.Y.Z-Linux-x86_64.AppImage --no-sandbox

# ARM64: add execute permission
chmod +x FyAgent-X.Y.Z-Linux-arm64.AppImage

# ARM64: if it still fails, try
./FyAgent-X.Y.Z-Linux-arm64.AppImage --no-sandbox
```
...
```bash
# x64
FYAGENT_GDK_BACKEND=wayland ./FyAgent-X.Y.Z-Linux-x86_64.AppImage

# ARM64
FYAGENT_GDK_BACKEND=wayland ./FyAgent-X.Y.Z-Linux-arm64.AppImage
```
...
FyAgent calculates Codex usage through **precise parsing based on JSONL session logs**, with normalized model names for consistent pricing lookup. New data aligns with official bills. If you still see older estimated data, delete the historical entries or wait for new session data to overwrite them.
...
See the current full disclaimer in [2.1 Add Provider → Codex OAuth Reverse Proxy](../2-providers/2.1-add.md#codex-oauth-reverse-proxy-claude-provider).
...
Yes. FyAgent covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `fyagent://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/NongHua123/fyagent/issues)
