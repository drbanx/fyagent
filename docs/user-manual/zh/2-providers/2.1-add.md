...
- 从他人分享获取
- 下载仓库随附的 [`deplink.html` 生成工具](https://github.com/NongHua123/fyagent/blob/main/deplink.html)，在本地打开后创建
...
FyAgent 提供 **Codex OAuth 反向代理**路径，让你可以**用 ChatGPT 账号**在 Claude Code 中复用 Codex 服务。
...
Codex OAuth 模型选择不只依赖硬编码列表。打开模型选择时，FyAgent 会按需从 ChatGPT Codex 后端拉取可用模型，默认映射仍可按需覆盖。
...
FyAgent 提供**完整 URL 端点模式**。默认情况下，FyAgent 会把配置的 `base_url` 视作**前缀**，再在其后拼接 `/v1/chat/completions` 等固定路径。对于部分厂商（如需要非标准 URL 布局的第三方服务），这种拼接方式会导致请求失败。
...
开关开启后显示的文本框可自定义自动压缩限制值。该开关仅在新增 Codex 供应商时显示；编辑已有供应商时可通过高级配置直接调整相关字段。
