...
完整应用仅使用无证书身份的 ad-hoc 签名；它没有 Apple Developer ID 签名，
也未经 Apple 公证，DMG 容器本身未签名。ad-hoc 签名不提供 Apple 信任。
请只从 [Releases 页面](https://github.com/NongHua123/fyagent/releases) 下载，核对
具体构建的发布证据，并按安全的
[Gatekeeper「仍要打开」流程](../1-getting-started/1.2-installation.md#gatekeeper-拦截首次启动时)
操作。不要关闭 Gatekeeper，也不要移除文件的 quarantine（隔离）属性。
...
**解决方法**：用专用环境变量 `FYAGENT_GDK_BACKEND` 切回原生 Wayland（该开关在 GTK 初始化前生效，钩子不会覆盖它）：
...
- 从桌面图标启动时，把它写进 `.desktop` 的 `Exec=` 行（如 `env FYAGENT_GDK_BACKEND=wayland /path/to/AppImage`），或在会话环境中设置，否则图标启动读不到该变量。
- 该变量是通用的：在 tiling Wayland 合成器（sway/Hyprland）下若反而出现点击失效，可设 `FYAGENT_GDK_BACKEND=x11`。
...
FyAgent 通过**精确解析 JSONL 会话日志**计算 Codex 用量，同时对模型名称做归一化以保证定价查询一致。新数据会与官方账单对齐；若仍看到旧的估算数据，可以删除历史条目或等待新会话数据覆盖。
...
当前完整免责声明参见 [2.1 添加供应商 → Codex OAuth 反向代理](../2-providers/2.1-add.md#codex-oauth-反向代理claude-供应商)。
...
1. 尝试切换主题（浅色/深色）
2. 重启应用
3. 删除 `~/.fyagent/settings.json` 重置设置
...
从系统托盘菜单切换"轻量模式"。主窗口关闭，FyAgent 仅作为托盘应用运行。再次切换或点击"打开主界面"即可退出。
...
可以。FyAgent 会覆盖所有窗口重新显示路径（正常启动、深链接、单例激活、托盘 `show_main` 以及轻量模式返程），点击 `fyagent://` 链接会**按需重建**主窗口并显示导入确认对话框。第一次打开会比普通状态略慢（需要重建窗口），但后续切换恢复正常速度。
...
1. 访问 [GitHub Issues](https://github.com/NongHua123/fyagent/issues)
2. 搜索是否有类似问题
3. 如果没有，创建新 Issue
4. 提供以下信息：
   - 操作系统和版本
...
FyAgent 的日志位于应用配置目录中。默认配置目录是用户主目录下的 `.fyagent`；如果你在高级设置中修改过配置目录，请以自定义目录为准。

提交 Issue 时，请按问题类型附上：

- 普通错误、网络或代理问题：`~/.fyagent/logs/` 中的 `fyagent.log` 及其轮转文件
- 应用崩溃：`~/.fyagent/crash.log` 及 `crash.log.1`、`crash.log.2`
- Windows 默认路径为 `C:\Users\<用户名>\.fyagent\...`
