...
export function SkillsPage() {
  ...
  return (
    <div className="fy-feature-page fy-skills-page">
                    ...
                    <Button
                      onClick={() =>
                        void write("ZIP 安装完成", async () => {
                          const path = await ports.skills.pickZip();
                          if (path)
                            await ports.skills.installFromZip(
                              path,
                              installTarget,
                            );
                        })
      ...
      {tab === "installed" ? (
        <>
          {installedQuery.error && installed.length > 0 && (
        ...
        <Discovery
          ...
          setDialog={setDialog}
          onInstalled={(skill) => {
            notify({ tone: "success", title: `${skill.name} 已安装` });
            void refreshAll();
          }}
        />
      )}
...
function Discovery({
  ...
  const install = async (skill: DiscoverableSkill) => {
    ...
    setBusy(true);
    try {
      onInstalled(await ports.skills.install(skill, installTarget));
    } catch (error) {
      notify({
        tone: "error",
        title: "安装失败",
  ...
  return (
    <section ref={resultsTop}>
      ...
      {(source === "repos" && discovery.isLoading) ||
      (source === "skillssh" && skillsSh.isFetching) ? (
        <EmptyState title="正在加载发现内容" description="请稍候">
          <Spinner />
        </EmptyState>
      ) : source === "repos" && discovery.error ? (
        <EmptyState
          title="无法加载仓库 Skills"
        ...
        />
      ) : source === "skillssh" && skillsSh.error ? (
        <EmptyState
        ...
        />
      ) : source === "repos" && (repos.data?.length ?? 0) === 0 ? (
        <EmptyState
          title="尚未配置仓库"
