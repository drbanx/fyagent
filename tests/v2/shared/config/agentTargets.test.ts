...
const expectedTargets: readonly AgentTargetDefinition[] = [
  ...
  },
  {
    ...
    toolId: "openclaw",
    name: "OpenClaw",
    scopeLabel: "自定义工作区 · secondary",
    instanceNames: ["secondary"],
    promptFile: "AGENTS.md",
    promptPath: "~/.openclaw/workspace-secondary/AGENTS.md",
    promptCanonicalResourceKey: "~/.openclaw/workspace-secondary/AGENTS.md",
    promptPathState: "create-on-enable",
    memoryDestination: "原生 MEMORY.md · USER.md",
    memorySyncEligibility: "verified-native",
    detected: false,
...
describe("V2 shared Agent target contract", () => {
  ...
  it("deduplicates repeated selections while retaining shared OpenClaw instances", () => {
    ...
    expect(groups).toEqual([
      ...
      {
        key: "~/.openclaw/workspace-secondary/AGENTS.md",
        primaryTargetId: "openclaw-secondary",
        targetIds: ["openclaw-secondary"],
        instanceNames: ["secondary"],
      },
    ]);
  ...
  it("marks only unverified Prompt files for creation on enable", () => {
    expect(
      agentTargets
        .filter((target) => target.promptPathState === "create-on-enable")
        .map((target) => target.id),
    ).toEqual(["gemini-global", "opencode-global", "openclaw-secondary"]);
  });
